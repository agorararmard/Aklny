...
session_start(); 
	...
	if($_GET)
	{
        ...
        $query3 = "SELECT ci.customization, ci.customId, ci.cnt,ci.deleted from `cart_item` ci WHERE ci.username ='".$userX."' AND ci.cartId = '".$cartX."' AND ci.rId = '".$restX."' AND ci.menu = '".$menuX."' AND ci.itemId = '".$itemX."'"; 
		//$pre_query = mysqli_real_escape_string($conn,$query);
	...
	if(isset($_POST["acart"]))
	{
        ...
        if(is_null($editArr["customization"]))
            $delquery = "DELETE FROM `cart_item` WHERE username = '".$userX."' AND cartId = '".$cartX."' AND rId = '".$restX."' AND menu = '".$menuX."' AND itemId = '".$itemX."' AND customId = '".$editArr["customId"]."' AND cnt = '".$editArr["cnt"]."'";
        else
            $delquery = "DELETE FROM `cart_item` WHERE username = '".$userX."' AND cartId = '".$cartX."' AND rId = '".$restX."' AND menu = '".$menuX."' AND itemId = '".$itemX."' AND customId = '".$editArr["customId"]."' AND cnt = '".$editArr["cnt"]."'AND customization = '".$editArr["customization"]."'";
...
<form class="" method="post" action="">
    ...
    <input type="radio" id="customRadio<?php echo $subArr["customId"];?>" name="customRadio" class="custom-control-input" value = "<?php echo $subArr["customId"];?>" <?php if($subArr["customId"]==$editArr["customId"]):?>checked<?php endif;?>>
	...
	<div class="input-group mb-3">
	  <input type="text" class="form-control" value="<?php echo $editArr["cnt"];?>" aria-label="Search" aria-describedby="basic-addon1" name="amount" id = "amount">
	</div>
	...
	<div class="input-group mb-3">
	  <input type="text" class="form-control" value ="<?php echo $editArr["customization"];?>" aria-label="Search1" aria-describedby="basic-addon2" name="customization" id = "customization">
	</div>
	...
	<div class="form-group container-login100-form-btn m-t-32 col">
		<input  name="delete"  type = "submit" class="login100-form-btn btn-danger" value = "Remove Item(s)">
	</div>
	
	<div class="form-group container-login100-form-btn m-t-32 col">
		<input  name="cancel"  type = "submit" class="login100-form-btn btn-secondary" value = "Cancel Changes">
	</div>
